...
locals {
  ...
  # computed
  aws_lambda_function_this = var.ignore_image_uri_changes ? aws_lambda_function.this_ignore_image_uri_changes : aws_lambda_function.this
}

resource "aws_lambda_function" "this" {
  count = local.create && var.create_function && !var.create_layer && !var.ignore_image_uri_changes ? 1 : 0
